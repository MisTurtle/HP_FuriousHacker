...
class TypingChallenge(TimedChallenge):
	...
	def __init__(self, chall_id: int, name: str, description: str, category: str, difficulty: int, container: ChallengeInterface, end_msg: Union[str, None], **kwargs):
		super().__init__(chall_id, name, description, category, difficulty, container, end_msg, **kwargs)
		...
		self.text_area.on("type", lambda: (self.start_timer(), self.show_restart_btn()) if self.text_area.get_content().__len__() == 1 else None)
		self.text_area.on("type", lambda: self.text_area.shake(6, 0.1, self.text_area.SHAKE_INSTANT))
		self.text_area.on("text_complete", lambda: self.end(min(1., self.perfect_min_time / self.get_timer().get_passed_time())))
